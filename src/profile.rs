...
/**
 * noahh profile list: List profiles of noahh
 * noahh profile switch: Switch main noahh profile
 * noahh profile add: Add noahh profile to the index
 * noahh profile remove: Remove noahh profile from the index
 * noahh profile rename: Rename noahh profile
 */
use std::path::Path;
...
fn is_valid_noahh_dir(_dir: &Path) -> bool {
	//TODO: this
	true
}
...
pub fn subcommand(cmd: Profile) {
	match cmd {
		...
		Profile::Add {
			...
			} else if !is_valid_noahh_dir(&location) {
				fail!("The specified path does not point to a valid Noahh installation");
			} else {
