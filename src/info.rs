...
pub enum Info {
	...
	/// Setup config (if you have manually installed Noahh)
	Setup {},
}
	...
	match cmd {
		Info::Set { field, value } => {
			...
			if field == "default-developer" {
				...
				fail!("Set the SDK Path using `noahh sdk set-path <PATH>`");
				return;
			} else if field == "index-token" {
				config.index_token = Some(value);
			} else if field == "index-url" {
				fail!("Set the Index URL using `noahh index set-url <URL>`");
				return;
			} else {
