...
pub fn noahh_root() -> PathBuf {
	// get data dir per-platform
	let data_dir: PathBuf;
	#[cfg(any(windows, target_os = "linux", target_os = "android"))]
	{
		data_dir = dirs::data_local_dir().unwrap().join("Noahh");
	};
	#[cfg(target_os = "macos")]
	{
		data_dir = PathBuf::from("/Users/Shared/Noahh");
	};
	#[cfg(not(any(
		windows,
		target_os = "macos",
...
impl Profile {
	...
	pub fn noahh_dir(&self) -> PathBuf {
		if self.platform == "win" {
			self.gd_path.parent().unwrap().join("noahh")
		} else if self.platform == "android32" || self.platform == "android64" {
			self.gd_path.join("game/noahh")
		} else {
			self.gd_path.join("Contents/noahh")
		}
	}

	pub fn mods_dir(&self) -> PathBuf {
		self.noahh_dir().join("mods")
	}
...
impl Config {
	...
	pub fn try_sdk_path() -> Result<PathBuf, String> {
		let sdk_var = std::env::var("NOAHH_SDK").map_err(|_| {
			"Unable to find Noahh SDK (NOAHH_SDK isn't set). Please install \
				it using `noahh sdk install` or use `noahh sdk set-path` to set \
				it to an existing clone. If you just installed the SDK using \
				`noahh sdk install`, please restart your terminal / computer to \
				apply changes."
		})?;
		...
		if !path.join("VERSION").exists() {
			return Err(
				"Internal Error: NOAHH_SDK/VERSION not found. Please reinstall \
				the Noahh SDK using `noahh sdk install --reinstall`"
					.into(),
			);
		}
	...
	pub fn cross_tools_path() -> PathBuf {
		noahh_root().join("cross-tools")
	}

	pub fn assert_is_setup(self) -> Config {
		if self.profiles.is_empty() {
			fatal!("No Noahh profiles found! Setup one by using `noahh config setup`");
		}
		self
	}
	...
	pub fn new() -> Config {
		if !noahh_root().exists() {
			return Config::default_fallback();
		}

		let config_json = noahh_root().join("config.json");
	...
	pub fn save(&self) {
		std::fs::create_dir_all(noahh_root()).nice_unwrap("Unable to create Noahh directory");
		std::fs::write(
			noahh_root().join("config.json"),
