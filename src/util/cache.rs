...
pub fn get_cache_bundle_from_dir(path: &Path) -> Option<CacheBundle> {
	path.join(".noahh_cache")
		.exists()
		.then(|| {
			let cache = ResourceCache::load(
				fs::read_to_string(path.join(".noahh_cache")).nice_unwrap("Unable to read cache"),
			);
			Some(CacheBundle {
				cache,
	...
	path.exists()
		.then(|| {
			match zip::ZipArchive::new(
				...
				Ok(mut archive) => {
					let cache: ResourceCache = if archive.by_name(".noahh_cache").is_ok() {
...
impl ResourceCache {
	...
	pub fn add_sheet(&mut self, sheet: &SpriteSheet, path: PathBuf) {
		if !path.is_relative() {
			unreachable!("Contact noahh developers: {}", path.display());
		}
