	...
	D: Deserializer<'de>,
{
	...
	// runtime by Noahh anyway so let's just crudely remove all 'v's for now
	...
	D: Deserializer<'de>,
{
	...
	// semver defaults to ^, noahh defaults to >=
...
pub trait ToNoahhString {
	fn to_noahh_string(&self) -> String;
}

impl ToNoahhString for VersionReq {
	fn to_noahh_string(&self) -> String {
		// noahh uses = instead of ^ for exact version
	...
	D: Deserializer<'de>,
{
	...
	match <HashMap<String, serde_json::Value>>::deserialize(value.clone()) {
		...
		Err(e) => {
			// Can be removed after Noahh hits v5
