...
fn uninstall() -> bool {
	...
	if !ask_confirm(
		&format!("Are you sure you want to uninstall Noahh SDK? (Installed at {sdk_path:?})"),
		false,
	) {
		fail!("Aborting");
		return false;
...
fn set_sdk_env(path: &Path) -> bool {
	...
	#[cfg(windows)]
	{
		...
		if hklm
			...
			.map(|(env, _)| env.set_value("NOAHH_SDK", &path.to_str().unwrap().to_string()))
			.is_err()
		{
			warn!(
				"Unable to set the NOAHH_SDK enviroment variable to {}",
				path.to_str().unwrap()
			);
			env_success = false;
		} else {
	...
	#[cfg(any(target_os = "linux", target_os = "android"))]
	{
		let shell = match detect_user_shell() {
			Some(s) => s,
			None => {
				warn!("Couldn't detect user shell. The CLI only supports bash, zsh and fish for setting the NOAHH_SDK environment variable at the moment.");
				return false;
			}
		};
...
fn get_linux_shell_info(shell: UserShell, path: &Path) -> Option<LinuxShellConfig> {
	...
	match shell {
		UserShell::Bash => Some(LinuxShellConfig {
			...
			regex: Regex::new("export NOAHH_SDK=.*").unwrap(),
			replace_with: format!("export NOAHH_SDK={}", path.to_str().unwrap()),
		}),
		...
		UserShell::Fish => Some(LinuxShellConfig {
			profile: format!("{}/.config/fish/conf.d/noahh.fish", home),
			profile_bak: format!("{}/.config/fish/conf.d/noahh.fish.bak", home),
			regex: Regex::new("set -gx NOAHH_SDK.*").unwrap(),
			replace_with: format!("set -gx NOAHH_SDK {}", path.to_str().unwrap()),
		}),
	}
}

fn get_sdk_path() -> Option<PathBuf> {
	if std::env::var("NOAHH_SDK").is_ok() && Config::try_sdk_path().is_ok() {
...
fn install(config: &mut Config, path: PathBuf, force: bool) {
	...
	if !force && std::env::var("NOAHH_SDK").is_ok() {
		if Config::try_sdk_path().is_ok() {
			...
			let env_sdk_path = std::env::var("NOAHH_SDK").unwrap();
			info!("NOAHH_SDK ({env_sdk_path}) is already set, but seems to point to an invalid sdk installation.");
	...
	let repo = clone_repo("https://github.com/noahh-sdk/noahh", &path)
		.nice_unwrap("Could not download SDK");

	// set NOAHH_SDK environment variable;
	if set_sdk_env(&path) {
		info!("Set NOAHH_SDK environment variable automatically");
	} else {
		warn!("Unable to set NOAHH_SDK environment variable automatically");
		info!(
			"Please set the NOAHH_SDK enviroment variable to {}",
			path.to_str().unwrap()
		);
	}
	...
	info!("Please restart your command line to have the NOAHH_SDK enviroment variable set.");
	info!("Use `noahh sdk install-binaries` to install pre-built binaries");
}
...
fn install_binaries(config: &mut Config, platform: Option<String>, version: Option<String>) {
	...
	if config.sdk_nightly {
		...
		info!("Installing binaries for {}", ver);
		release_tag = format!("v{}", ver);
		// remove any -beta or -alpha suffixes as noahh cmake doesn't care about those
		let mut stripped_ver = ver.clone();
		stripped_ver.pre = Prerelease::EMPTY;
		target_dir = Config::sdk_path().join(format!("bin/{}", stripped_ver));
	}
	...
	let res = reqwest::blocking::Client::new()
		.get(format!(
			"https://api.github.com/repos/noahh-sdk/noahh/releases/tags/{}",
			release_tag
		))
		...
		.nice_unwrap(format!("Could not parse Noahh release \"{}\"", release_tag));
	...
	for asset in res.assets {
		...
		// skip resources
		if !asset.name.to_lowercase().contains("noahh") {
			continue;
		}
...
fn set_sdk_path(path: PathBuf, do_move: bool) {
	if do_move {
		let old = std::env::var("NOAHH_SDK")
			.map(PathBuf::from)
			.nice_unwrap("Cannot locate SDK.");

		assert!(
			old.is_dir(),
			"Internal Error: NOAHH_SDK doesn't point to a directory ({}). This \
			might be caused by having run `noahh sdk set-path` - try restarting \
			your terminal / computer, or reinstall using `noahh sdk install --reinstall`",
			old.display()
		);
		assert!(
			old.join("VERSION").exists(),
			"Internal Error: $NOAHH_SDK/VERSION not found. Please reinstall the Noahh SDK."
		);
		assert!(
			!path.exists(),
		...
		assert!(
			...
			"{} is either malformed or not a Noahh SDK installation",
			path.to_str().unwrap()
		);
	}
...
fn download_xwin(dest: &Path) -> Result<(), Box<dyn std::error::Error>> {
	let resp = reqwest::blocking::Client::builder()
		.user_agent(format!("noahh-cli/{}", env!("CARGO_PKG_VERSION")))
		.build()?
...
fn install_linux(
	...
	if toolchain_path.exists() {
		...
		clone_repo(
			"https://github.com/noahh-sdk/clang-msvc-sdk",
			&toolchain_path,
		)
...
pub fn subcommand(cmd: Sdk) {
	match cmd {
			...
			if !force {
				if let Some(path) = get_sdk_path() {
					fatal!(
						...
						update the SDK, use `noahh sdk update`, or if you \
			...
			let actual_path = match path {
				...
				None => {
					let default_path = if cfg!(target_os = "macos") {
						PathBuf::from("/Users/Shared/Noahh/sdk")
					} else {
						dirs::document_dir()
							.nice_unwrap(
								"No default path available! \
								Please provide the path manually as an \
								argument to `noahh sdk install`",
							)
							.join("Noahh")
					};
					if !confirm!(
		...
		Sdk::Version => info!("Noahh SDK version: {}", get_version()),
