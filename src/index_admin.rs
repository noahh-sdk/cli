...
fn get_pending_mods(page: i32, config: &Config) -> PaginatedData<PendingMod> {
	...
	let response = client
		...
		.send()
		.nice_unwrap("Failed to connect to the Noahh Index");

	if !response.status().is_success() {
		if let Ok(body) = response.json::<ApiResponse<String>>() {
			warn!("{}", body.error);
		}
		fatal!("Bad response from Noahh Index");
	}

	let data: ApiResponse<PaginatedData<PendingMod>> = response
		.json()
		.nice_unwrap("Failed to parse response from the Noahh Index");
...
fn download_mod(version: &PendingModVersion, id: &str, config: &Config) {
	...
	let mod_path = mods_dir.join(format!("{}.noahh", version.mod_id));
