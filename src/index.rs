...
pub enum MyModAction {
	/// Create a new mod
	Create {
		/// Direct download link to the .noahh file
...
pub fn install_mod(
	...
	fs::write(&dest, bytes).nice_unwrap("Unable to install .noahh file");
...
fn submit(action: MyModAction, config: &mut Config) {
	...
	let download_link =
		download_link.unwrap_or_else(|| ask_value("Download URL for the .noahh file", None, true));
	...
	if is_update {
		info!("Fetching mod id from .noahh file");
		...
		let mut zip_archive =
			zip::ZipArchive::new(zip_data).nice_unwrap("Unable to decode .noahh file");
...
fn create_mod(download_link: &str, config: &mut Config) {
	...
	if !response.status().is_success() {
		let body: ApiResponse<String> = response
			.json()
			.nice_unwrap("Unable to parse response from Noahh Index");
