...
fn create_template(template: CreateTemplate) {
	...
	let (used_template, branch) = if template.template.contains('@') {
		...
		("https://github.com/noahh-sdk/example-mod", "main")
	} else {
		(template.template.as_str(), "main")
	};
	...
	// Download the action from https://raw.githubusercontent.com/noahh-sdk/build-noahh-mod/main/examples/multi-platform.yml
	if template.action {
		...
		let action = reqwest::blocking::get("https://raw.githubusercontent.com/noahh-sdk/build-noahh-mod/main/examples/multi-platform.yml").nice_unwrap("Unable to download action");
		fs::write(
			action_path,
	...
	let mod_json_content: String = {
		if mod_json_path.exists() {
			...
			mod_json
				.replace("$NOAHH_VERSION", &get_version().to_string())
...
pub fn build_template(location: Option<PathBuf>) {
	...
	let template_options = [
		(
			...
			"https://github.com/noahh-sdk/example-mod",
		),
		(
			"Minimal - Minimal mod with only the bare minimum to compile.",
			"https://github.com/noahh-sdk/example-mod@minimal",
		),
		(
	...
	if config.default_developer.is_none() {
		info!(
			...
			`noahh config set default-developer <name>`",
			&final_developer
		);
