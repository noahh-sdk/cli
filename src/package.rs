...
pub enum Package {
	/// Install a .noahh package to the current profile
	Install {
		/// Location of the .noahh package to install
		path: PathBuf,
	},

	/// Create a .noahh package
	New {
		...
		/// {mod.id}.noahh and placed at the root path
		#[clap(short, long)]
	...
	/// Currently just an alias for `noahh project check`, will be removed in
	/// CLI v3.0.0!
...
fn create_package(
	...
	// path to the final .noahh file
	let mut output = raw_output.unwrap_or(root_path.join(format!("{}.noahh", mod_file_info.id)));

	// If it's a directory, add file path to it
	if output.is_dir() {
		output.push(&mod_file_info.id);
		output.set_extension("noahh");
		warn!(
	...
	// Setup cache from the previously built .noahh archive
