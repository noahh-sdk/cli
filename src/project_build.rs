...
pub fn build_project(
    ...
    configure_only: bool,
    build_only: bool,
    _ninja: bool, // ignored now — Ninja is always used on Windows
    ndk_path: Option<String>,
    config_type: Option<String>,
    extra_conf_args: Vec<String>,
) {
    if !Path::new("CMakeLists.txt").exists() {
        fatal!("Could not find CMakeLists.txt. Please run this within a Noahh project!");
    }
    ...
    match platform {
        PlatformName::Windows => {
            // ⭐ ALWAYS use Ninja + Clang-CL on Windows
            conf_args.extend(["-G", "Ninja"].map(String::from));
            conf_args.push("-DCMAKE_C_COMPILER=clang-cl".into());
            conf_args.push("-DCMAKE_CXX_COMPILER=clang-cl".into());
            ...
            if !build_only {
                let ndk_path = ndk_path.unwrap_or_else(|| {
                    std::env::var("ANDROID_NDK_ROOT").nice_unwrap(
                        "Failed to get NDK path, either pass it through --ndk or set ANDROID_NDK_ROOT"
                    )
                });
                ...
                // Always use Ninja for Android builds on Windows
                if cfg!(target_os = "windows") && !extra_conf_args.contains(&"-G".to_owned()) {
                    conf_args.extend(["-G", "Ninja"].map(String::from));
                }

                conf_args.push("-DCMAKE_EXPORT_COMPILE_COMMANDS=1".into());
                conf_args.push("-DNOAHH_DONT_INSTALL_MODS=1".into());
            }
        }
        ...
        PlatformName::Ios => {
            ...
            conf_args.push("-DNOAHH_TARGET_PLATFORM=iOS".into());
            conf_args.push("-DNOAHH_DONT_INSTALL_MODS=1".into());
        }
    }
    ...
    if !build_only {
        let status = Command::new("cmake")
            ...
            .wait()
            .nice_unwrap("Failed to wait for cmake");
        if !status.success() {
    ...
    if !configure_only {
        let status = Command::new("cmake")
            ...
            .wait()
            .nice_unwrap("Failed to wait for cmake build");
        if !status.success() {
