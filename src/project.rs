...
use crate::mod_file::{PlatformName, ToNoahhString};
...
pub enum Project {
	/// Initialize a new Noahh project (same as `noahh new`)
	New {
	...
	Check {
		...
		/// directory. A directory called noahh-deps will be created inside
...
fn clear_cache(dir: &Path) {
	...
	if let Some(dir) = dir {
		for file in fs::read_dir(dir).nice_unwrap("Unable to read build directory") {
			...
			if ext == "noahh" {
				fs::remove_file(path).nice_unwrap("Unable to delete cached .noahh package");
			}
		}
	} else {
		warn!(
			"Unable to find cached .noahh package, can't clear it. It might be \
...
fn find_index_dependency(dep: &Dependency, config: &Config) -> Result<Found, String> {
	...
	let found = index::get_mod_versions(
		...
		config,
		false,
		Some(dep.version.to_noahh_string()),
	)?;
...
pub fn check_dependencies(
	...
	let dep_dir = output.join("noahh-deps");
	...
	for dep in &mod_info.dependencies {
		...
		if let Found::Some(inst_path, inst_info) = found_existing {
			...
			path_to_dep_noahh = inst_path;
			_noahh_info = inst_info;
		} else {
			// check index
			...
			match (found_in_installed, found_in_index) {
				...
				(Found::Wrong(version), Found::Some(path, indx_info)) => {
					...
					let noahh_path = config
						.get_current_profile()
						.mods_dir()
						.join(format!("{}.noahh", indx_info.id));
					std::fs::copy(path, &noahh_path).nice_unwrap("Failed to install .noahh");
					path_to_dep_noahh = noahh_path;
					_noahh_info = indx_info;
				}
			...
			// 	if info.version == noahh_info.version {
			// 		continue;
			// 	}
			// }
		}

		// unzip the whole .noahh package because there's only like a few
		// extra files there aside from the lib, headers, and resources
		zip::ZipArchive::new(fs::File::open(path_to_dep_noahh).unwrap())
			.nice_unwrap("Unable to unzip")
			.extract(dep_dir.join(&dep.id))
			.nice_unwrap("Unable to extract noahh package");

		// add a note saying if the dependencey is required or not (for cmake to
		// know if to link or not)
		fs::write(
			dep_dir.join(&dep.id).join("noahh-dep-options.json"),
